...
sup1.Vmax = 24*60            #[m**3/day] Extraction permit
...
sup2.Vmax = 24*60            #[m**3/day] Extraction permit
...
simu.dt = 60*60       #[sec] sample time
simu.simTime = 7*24*3600 #[sec] Sim time 
## Demand and electricity prices ####################
mat1 = scipy.io.loadmat('+ctrlPumpingStation/UserConsumption.mat')  # Simulated demand
mat2 = scipy.io.loadmat('Elspotprice3mdr.mat')  # Actual electricity prices from https://www.energidataservice.dk/tso-electricity/elspotprices
simu.d = mat1['q_u1'][::4] #User consumption every 15 min (so addapt to other dt when necessary)
simu.TIME = mat1['time'][::4]
c0 = mat2['price'] / 1000 #Electricity price hour for hour per kWh
simu.c = c0#np.repeat(c0, 4) #Electricity prices every hour (so addapt to other dt when necessary)
...
simu.ite = 3*simu.M           #Should be multiple of M for nice plotting :)
if simu.ite > MaxIte:
