...
pub const ROSENBROCK_NUM_DECISION_VARIABLES: usize = 5;

/// Number of parameters
pub const ROSENBROCK_NUM_PARAMETERS: usize = 2;

/// Number of parameters associated with augmented Lagrangian
pub const ROSENBROCK_N1: usize = 0;

/// Number of penalty constraints
pub const ROSENBROCK_N2: usize = 2;
...
const CONSTRAINTS_BALL_RADIUS : f64 = 1.5;
...
pub fn initialize_solver() -> AlmCache {
    let panoc_cache = PANOCCache::new(ROSENBROCK_NUM_DECISION_VARIABLES, EPSILON_TOLERANCE, LBFGS_MEMORY);
    AlmCache::new(panoc_cache, ROSENBROCK_N1, ROSENBROCK_N2)
}
...
fn unscale_result(solver_status: &mut Result<AlmOptimizerStatus, SolverError>) {
    if let Ok(sss) = solver_status {
        let w_cost : f64 = icasadi_rosenbrock::get_w_cost();
...
pub fn solve(
    ...
    assert_eq!(p.len(), ROSENBROCK_NUM_PARAMETERS, "Wrong number of parameters (p)");
    assert_eq!(u.len(), ROSENBROCK_NUM_DECISION_VARIABLES, "Wrong number of decision variables (u)");

    // Start by initialising the optimiser interface (e.g., set w=1)
    icasadi_rosenbrock::init_rosenbrock();
    ...
    let psi = |u: &[f64], xi: &[f64], cost: &mut f64| -> Result<(), SolverError> {
        icasadi_rosenbrock::cost(u, xi, p, cost);
        Ok(())
    };
    let grad_psi = |u: &[f64], xi: &[f64], grad: &mut [f64]| -> Result<(), SolverError> {
        icasadi_rosenbrock::grad(u, xi, p, grad);
        Ok(())
    };
    
    let f2 = |u: &[f64], res: &mut [f64]| -> Result<(), SolverError> {
        icasadi_rosenbrock::mapping_f2(u, p, res);
        Ok(())
