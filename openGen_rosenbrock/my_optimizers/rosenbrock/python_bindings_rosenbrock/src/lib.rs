///
/// Auto-generated python bindings for optimizer: rosenbrock
///
...
#[pymodule]
fn rosenbrock(_py: Python, m: &PyModule) -> PyResult<()> {
...
impl Solver {
    ...
    fn run(
        ...
        let mut u = [0.0; ROSENBROCK_NUM_DECISION_VARIABLES];

        // ----------------------------------------------------
        // Set initial value
        // ----------------------------------------------------
        if let Some(u0) = initial_guess {
            if u0.len() != ROSENBROCK_NUM_DECISION_VARIABLES {
                println!(
                    "1600 -> Initial guess has incompatible dimensions: {} != {}",
                    u0.len(),
                    ROSENBROCK_NUM_DECISION_VARIABLES
                );
                return Ok(None);
            }
        ...
        // Check parameter
        // ----------------------------------------------------
        if p.len() != ROSENBROCK_NUM_PARAMETERS {
            println!(
                "3003 -> wrong number of parameters: {} != {}",
                p.len(),
                ROSENBROCK_NUM_PARAMETERS
            );
            return Ok(None);
        }
